...
{
    if (!read_f32(path, out) || out.size() != count)
    {
...
static std::vector<float> codes_to_decoder_hidden(const std::string& wdir, const std::vector<int>& codes, int frames, int threads)
{
    constexpr int Q = 16;
...
static bool write_f32_file(const std::string& path, const std::vector<float>& data)
{
...
static std::vector<float> run_decoder_chunk(ncnn::Net& net, const std::vector<float>& hidden, int frames, int chunk_frames)
{
    constexpr int DIM = 1024;
    constexpr int SAMPLES_PER_FRAME = 1920;
    std::vector<float> chunk((size_t)chunk_frames * DIM, 0.0f);
    std::memcpy(chunk.data(), hidden.data(), (size_t)frames * DIM * sizeof(float));

    ncnn::Extractor ex = net.create_extractor();
    ex.input("in0", mat2_from_vec(chunk, DIM, chunk_frames));
    ncnn::Mat out;
    if (ex.extract("out0", out) != 0)
    {
        std::fprintf(stderr, "decoder extract failed\n");
        std::exit(1);
    }
    const int keep = frames * SAMPLES_PER_FRAME;
    const float* p = (const float*)out.data;
    return std::vector<float>(p, p + keep);
}

static void compare_wav(const std::vector<float>& got, const std::string& ref_path)
{
    std::vector<float> ref;
    if (!read_f32(ref_path, ref)) return;
...
int main(int argc, char** argv)
{
    if (argc != 21)
    {
        std::fprintf(stderr, "Usage: %s <talker_prefill_param> <talker_decode_param> <talker_bin> <prefill_embed> <prefill_mask> <prefill_cos> <prefill_sin> <codepred_body_dir> <codepred_weights_dir> <ref_codes_i32> <frames> <tts_pad_embed_f32> <out_codes_i32> <front_weights_dir> <decoder_param> <decoder_bin> <ref_wav_f32> <out_hidden_f32> <out_wav_f32> <threads>\n", argv[0]);
        return 2;
    }
    ...
    const int threads = std::atoi(argv[20]) > 0 ? std::atoi(argv[20]) : 4;
    const int dim = 1024;
    ...
    const std::string front_weights_dir = argv[14];
    const std::string decoder_param = argv[15];
    const std::string decoder_bin = argv[16];
    const std::string ref_wav = argv[17];
    const std::string out_hidden = argv[18];
    const std::string out_wav = argv[19];

    std::vector<float> decoder_hidden = codes_to_decoder_hidden(front_weights_dir, generated, frames, threads);
    if (!write_f32_file(out_hidden, decoder_hidden))
    {
    ...
    decoder_net.opt.num_threads = threads;
    decoder_net.opt.use_vulkan_compute = false;
    decoder_net.opt.use_packing_layout = false;
    if (decoder_net.load_param(decoder_param.c_str()) != 0 || decoder_net.load_model(decoder_bin.c_str()) != 0)
    {
        std::fprintf(stderr, "failed to load decoder graph\n");
        return 1;
    }
    std::vector<float> wav = run_decoder_chunk(decoder_net, decoder_hidden, frames, 325);
    if (!write_f32_file(out_wav, wav))
    {
        std::fprintf(stderr, "failed to write %s\n", out_wav.c_str());
        return 1;
    }
    std::printf("full_pipeline wav_samples=%zu wrote=%s\n", wav.size(), out_wav.c_str());
    compare_wav(wav, ref_wav);
    return 0;
}
